...
def augment_character_set(data, character_set):
    ...
    return np.vstack((data, np.array([rotation_90, rotation_180, rotation_270], dtype=object)))
...
class OmniglotData(object):
    ...
    def _generate_random_task(self, images, character_indices, shot, way, eval_samples):
        ...
        for character in task_characters:
            # print(type(character_indices), character_indices, character)
            # print(np.where(character_indices == character)[0])
            # print(type(images))
