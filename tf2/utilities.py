...
def sample_normal(mu, log_variance, num_samples):
    ...
    """
    shape = tf.concat([tf.constant([num_samples]), tf.shape(input=mu)], axis=-1)
...
def multinoulli_log_density(inputs, logits):
    ...
    """
    return -tf.nn.softmax_cross_entropy_with_logits(labels=tf.stop_gradient(inputs), logits=logits)
...
def gaussian_log_density(inputs, mu, logVariance):
    ...
    """
    d = tf.cast(tf.shape(input=inputs)[-1], tf.float32)
    xc = inputs - mu
    return -0.5*(tf.reduce_sum(input_tensor=(xc * xc) / tf.exp(logVariance), axis=-1)
                 + tf.reduce_sum(input_tensor=logVariance, axis=-1) + d * tf.math.log(2.0*np.pi))
...
def conv2d_pool_block(inputs, use_batch_norm, dropout_keep_prob, pool_padding, name):
    ...
    h = tf.compat.v1.layers.max_pooling2d(inputs=h, pool_size=[2, 2], strides=2, padding=pool_padding, name=(name + '_pool'))
...
def dense_layer(inputs, output_size, activation, use_bias, name):
    ...
    return tf.compat.v1.layers.dense(
        ...
        kernel_initializer=xavier_initializer(uniform=False),
        use_bias=use_bias,
        bias_initializer=tf.compat.v1.random_normal_initializer(stddev=1e-3),
